...
test("render all options passed to Select", () => {
  ...
  expect(rendered.getAllByRole("menuitemradio")).toHaveLength(options.length);
});
...
test("calls the onOptionSelected prop with the selected option and its index if passed", () => {
  ...
  const { getAllByRole, getByTestId } = render(
    <Select options={options} onSelect={onOptionSelected} />
  );
...
test("can customize select label", () => {
  const { getByText } = render(
    <Select options={options} placeHolder="THIS IS A CUSTOM LABEL" />
  );
